...
export class JobRepositories {
    public async GetAllJobs(): Promise<Job[] | null> {
        ...
        try {
            const res = await fetch(`${BACKEND_URL}/job/getall`);
            if (!res.ok) {
    ...
    public async CreateJob(job: Job): Promise<Job | null> {
        const BACKEND_URL = process.env.BACKEND_URL;
        try {
    ...
    public async UpdateJob(job: Job): Promise<Job | null> {
        const BACKEND_URL = process.env.BACKEND_URL;
        try {
            const res = await fetch(`${BACKEND_URL}/job/update`, {
                method: "PUT",
                headers: {
                    "Content-Type": "application/json",
                },
